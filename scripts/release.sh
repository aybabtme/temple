...
GOOS=linux go build -ldflags "-X main.version=$VERSION" -o /tmp/temple_build/linux/temple ../
pushd /tmp/temple_build/linux/
tar cvzf /tmp/temple_build/temple_linux.tar.gz temple
popd

mkdir -p /tmp/temple_build/darwin
GOOS=darwin go build -ldflags "-X main.version=$VERSION" -o /tmp/temple_build/darwin/temple ../
